function CheckboxBoard({
    ...
    for (let i = 0; i < value.height; i++) {
        for (let j = 0; j < value.width; j++) {
            board.push((
                <input key={`checkbox${i}x${j}`} type={"checkbox"} checked={value.board[i][j]} onChange={(e) => {
                    console.log(i, j, value.board, e.target.checked);
                    value.board[i][j] = e.target.checked;
                    console.log(value);
                    onChange({
                        width: value.width,
                        height: value.height,
    ...
    return (
        <div>
            <div>
                {infoText}
                <input type="number" size="2" value={value.width} onChange={(e) => {
                ...
                X
                <input type="number" size="2" value={value.height} onChange={(e) => {
