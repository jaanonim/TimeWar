...
function AddArmyPage() {
    ...
    const send = async () => {
        ...
        let response = await fetch((environment === "development" ? developmentUrl : productionUrl) + "admin/addArmy", {
            method: "POST",
    ...
    return (<>
            ...
            <form>
                ...
                <label>Max lives <input type="number" value={maxLives}
                                        onChange={(e) => setMaxLives(e.target.value)}/></label>
                <label>Damage <input type="number" value={damage} onChange={(e) => setDamage(e.target.value)}/></label>
                <label>IsFlyable <input type="checkbox" checked={isFlyable}
                                        onChange={(e) => setIsFlyable(e.target.checked)}/></label>
                <label>Model <input type="text" value={model} onChange={(e) => setModel(e.target.value)}/></label>
                <label>Scale <input type="number" value={scale} onChange={(e) => setScale(e.target.value)}
                                    step="0.01"/></label>
                <label>Price <input type="number" value={price} onChange={(e) => setPrice(e.target.value)}/></label>
                <CheckboxBoard infoText="Move Mask" value={moveMask} onChange={setMoveMask}/>
                <CheckboxBoard infoText="Attack Mask" value={attackMask} onChange={setAttackMask}/>
